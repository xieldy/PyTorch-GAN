    ...
    dataloader = torch.utils.data.DataLoader(
        datasets.__dict__[opt.dataset](
            ...
            transform=transforms.Compose(
                [transforms.Resize(img_size),transforms.CenterCrop(img_size), transforms.ToTensor(),transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
            ), 
        ),
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        z = Variable(FloatTensor(np.random.normal(0, 1, (batch_size, opt.latent_dim)))) #均值0，标准差1，(batch_size, opt.latent_dim)=shape
